...
class Application_Model_Kernel_Article extends Application_Model_Kernel_Page
{
    ...
    )
    {
        parent::__construct($idPage, $idRoute, $idContentPack,
            $pageEditDate, $pageStatus, self::TYPE_ARTICLE,
            $position);
        $this->id = $id;
...
//		}
    ...
    public static function getList($order, $orderType, $content, $route, $searchName, $status, $page, $onPage, $limit, $group = true, $wher = false, $area = false, $nextorder = false)
    {
        ...
        $select->where('pages.pageType = ?', self::TYPE_ARTICLE);
        if ($wher) {
            $select->where($wher);
        }
        if ($order && $orderType) {
